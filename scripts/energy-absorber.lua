...
function energy_absorber.on_placed(e)
  ...
  if not equipment.valid or equipment.name ~= "energy-absorber-equipment" then
    return
  end
  local grid = e.grid
  ...
  if grid.get_contents()["energy-absorber-equipment"] > 1 then
    ...
    if cursor_stack and cursor_stack.valid_for_read and cursor_stack.name == "energy-absorber-equipment" then
    ...
    elseif cursor_stack and player.clear_cursor() then
      cursor_stack.set_stack({ name = "energy-absorber-equipment", count = 1 })
    end
