...
if not remote.interfaces["kr-radioactivity"] then
  remote.add_interface("kr-radioactivity", {
    ...
    remove_radioactive_entity = function(entity_name)
      if type(entity_name) ~= "string" then
        error("Value for 'entity_name' must be a string.")
      end
    ...
    add_radioactive_item = function(item_name)
    ...
    remove_radioactive_item = function(item_name)
      if type(item_name) ~= "string" then
        error("Value for 'item_name' must be a string.")
      end
    ...
    set_radioactive_area_offset = function(radius)
...
function doRadioactiveDamage(player)
  if player.valid and player.character and player.character.valid then
    ...
    if character.grid and character.grid.valid and character.grid.shield > 0 then
      local max_shield_perc = character.grid.max_shield * 0.08
      if max_shield_perc >= base_damage then
        base_damage = base_damage + max_shield_perc
      else
    ...
    if character and character.valid then
      ...
      player.remove_alert({ icon = { type = "virtual", name = "kr-nuclear-2" }, message = {
        "other.kr-taking-radioactive-damage",
      } })
    end
  end
      ...
      if character and character.valid then
        ...
        if
          player.surface.count_entities_filtered({
            ...
            area = {
              {
                position.x - global.krastorio.radioactive_area_offset,
                position.y - global.krastorio.radioactive_area_offset,
              },
              {
                position.x + global.krastorio.radioactive_area_offset,
                position.y + global.krastorio.radioactive_area_offset,
