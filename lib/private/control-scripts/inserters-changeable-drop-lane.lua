...
function changeInserterDropLane(event)
  ...
  if inserter and inserter.type == "inserter" then
    -- Change lane
    local delta_drop_pos = {
      x = inserter.drop_position.x - inserter.position.x,
      y = inserter.drop_position.y - inserter.position.y,
    }
...
local function onEntitySettingsPasted(event)
  ...
  if source and destination and source.valid and destination.valid then
    if source.type == "inserter" and (source.type == destination.type) then
      if
        (source.prototype.inserter_pickup_position[1] ~= destination.prototype.inserter_pickup_position[1])
        or (source.prototype.inserter_pickup_position[2] ~= destination.prototype.inserter_pickup_position[2])
        or (source.prototype.inserter_drop_position[1] ~= destination.prototype.inserter_drop_position[1])
        or (source.prototype.inserter_drop_position[2] ~= destination.prototype.inserter_drop_position[2])
      then
