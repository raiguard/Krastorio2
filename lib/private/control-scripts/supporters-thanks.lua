...
local function createThanksList()
  ...
  for i, supporter in pairs(patreons_list.getHighSupportersList()) do
    list = list .. "[font=heading-2]" .. supporter .. " [/font], "
  end
  return list
end
...
local function thanksOnPlayerJoin(event)
  if
    (event.name == defines.events["on_cutscene_cancelled"] and event.player_index <= 1)
    or (event.name == defines.events["on_player_joined_game"] and event.player_index > 1)
  then
    ...
    if not (global.thanks_printed[player.name] == true) then
      ...
      player.print({ "other.wiki-info" }, { 1.0, 1.0, 0.2, 1.0 })
    end
  end
end
